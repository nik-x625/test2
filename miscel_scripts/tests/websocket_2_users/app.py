...
async def start(websocket):
    ...
    print('# in start, join_key created: '+str(join_key))
    JOIN[join_key] = game, connected

    watch_key = secrets.token_urlsafe(12)
    print('# in start, watch_key created: '+str(watch_key))
...
async def join(websocket, join_key):
    ...
    """
    print('# in join, join_key: '+str(join_key)+'   current JOIN list: '+str(JOIN))
...
async def watch(websocket, watch_key):
    ...
    print('# in watch, watch_key: '+str(watch_key))
    try:
...
async def handler(websocket):
    ...
    print('# in handler, event: '+str(event))
